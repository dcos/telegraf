...
// MesosContainers describes the options available to this plugin
type MesosContainers struct {
...
func (mc *MesosContainers) SampleConfig() string {
	return sampleConfig
}

// Description returns a one-sentence description of mesos_containers
func (mc *MesosContainers) Description() string {
...
func (mc *MesosContainers) Gather(acc telegraf.Accumulator) error {
	cli := httpagent.NewSender(mc.getClient().Send)
	ctx, cancel := context.WithTimeout(context.Background(), mc.Timeout.Duration)
	defer cancel()

	gc, err := mc.getContainers(ctx, cli)
	if err != nil {
		return err
	}
...
func (mc *MesosContainers) getContainers(ctx context.Context, cli calls.Sender) (*agent.Response_GetContainers, error) {
...
// getClient returns the *httpcli.Client that is a member of mc. If it hasn't been created yet, it is created and then
// returned.
func (mc *MesosContainers) getClient() *httpcli.Client {
	if mc.client == nil {
		mc.client = httpcli.New(httpcli.Endpoint(mc.MesosAgentUrl + "/api/v1"))
	}
	return mc.client
}
...
func init() {
	inputs.Add("mesos_containers", func() telegraf.Input {
